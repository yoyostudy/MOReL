...
class ActorCriticPolicy(nn.Module):
    def __init__(self, input_dim,
        ...
        self.var = torch.nn.Parameter(torch.tensor([0.0,0.0,0.0,0.0]).cuda(), requires_grad = True)
    ...
    def forward(self, obs, action = None):
        ...
        # Create covariance matrix using torch.diag_embed
        var = torch.diag_embed(var)
        action_dist = self.distribution(mean, var)
